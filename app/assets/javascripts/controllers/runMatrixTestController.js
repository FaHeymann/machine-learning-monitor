export default class RunTestController {
  ...
  getTestAmount() {
    let product = 1;
    this.labels.forEach((label) => { product *= label.include === 'variable' ? 2 : 1; });
    this.parameters.forEach((parameter) => {
      if (parameter.type === 'ENUM') {
        product *= parameter.value.length;
      } else {
        product *= (parameter.value.match(/;/g) || []).length + 1;
      }
    });
    return product;
  }
  ...
  static getParameterDefaultValue(parameter) {
    switch (parameter.type) {
      case 'ENUM': return [parameter.enumValues[0]];
